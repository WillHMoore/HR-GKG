...
AMNESTYdata <- read.csv("HR-GKG.AMNESTY.gkgv2.csv", sep="\t")

# make list object of the Counts field
dat <- as.list(AMNESTYdata$Counts)

# apply function to list object (Counts field)
dat2 <- lapply(dat, foo, ";")

# make recordID vector for data.frame
recordID <- rep(1:nrow(AMNESTYdata), unlist(lapply(dat2, length)))
...
write.csv(dat4, "AI-EventsData.csv", row.names=F)
